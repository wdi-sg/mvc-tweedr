...
module.exports = (dbPoolInstance) => {
    ...
    return {
        ...
        getMyTweets: (userId, callback) => {
            ...
            SELECT id, screen_name, avatar, tweetmsg, timestamp_col  FROM listTweets ORDER BY timestamp_col`;
        ...
        getAll: (callback) => {
            let query = 'SELECT users.id, users.screen_name, users.avatar, tweets.tweetmsg, tweets.timestamp_col FROM users INNER JOIN tweets ON (tweets.userid = users.id) ORDER BY tweets.timestamp_col ASC';
            dbPoolInstance.query(query, (error, queryResult) => {
                if (error) {
                    // invoke callback function with results after query has executed
                    callback(error, null);
