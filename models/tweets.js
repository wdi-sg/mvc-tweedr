module.exports = (dbPoolInstance) => {
    ...
    let getAll = (req,callback) => {
        ...
        let query = 'SELECT DISTINCT tweets.content, users.username,tweets.id AS tweets_id, tweets.users_id, timestamp FROM users INNER JOIN tweets ON (users.id = tweets.users_id) INNER JOIN follows ON (tweets.users_id = follows.users_id) WHERE follows.f_er_users_id =$1 OR tweets.users_id=$1 ORDER BY timestamp';
    ...
    // PAIRING EXERCISE
    let like = (req,callback) => {
        console.log('IM AT MODELS')
        let values = [req.cookies.userId];
        let query = 'INSERT INTO likes (tweets_id,users_id) VALUES ($1,$2) RETURNING *';
        dbPoolInstance.query(query,values,(err,res)=>{
            if (err) {
                callback(err,null);
            } else {
