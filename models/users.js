...
module.exports = (dbPoolInstance) => {
    ...
    const getFollowers = (ofUser, callback) => {

        let query = `SELECT users.* FROM users INNER JOIN users_followers ON users.user_id = users_followers.follower_id WHERE leader_id = ${ofUser}`;
    ...
    const getFollowing = (currentUserId, callback) => {

        let query = `SELECT users.* FROM users INNER JOIN users_followers ON users.user_id = users_followers.leader_id WHERE follower_id = ${currentUserId}`;
        dbPoolInstance.query(query, (err, result) => {
            callback(err, result.rows);
        });
    }
